...
func (c *serviceEntryChecker) Check(ctx context.Context, state *discovery.ClusterState) ([]Finding, error) {
	...
	for _, se := range state.ServiceEntries {
		...
		if len(hosts) > 256 {
			findings = append(findings, Finding{
				...
				Kind:         se.Kind,
				Message:      fmt.Sprintf("ServiceEntry has %d hosts, which exceeds the limit of 256 in OSSM 3.x", len(hosts)),
				...
				Remediation:  RemediationGuide{
					...
					DocsLinks:   []string{"https://docs.redhat.com/en/documentation/openshift_container_platform/4.14/html/service_mesh/migrating-from-service-mesh-2-to-service-mesh-3#disable-add-ons-and-reconfigure-replacements"},
				},
			})
		}
